...
public class Simulator
{
    ...
    private String[] commands = {"M", "Z", "<", ">", "r", "e", "0", "1", "2", "3", "4", "5", "6", "7", "U", "R", "D", "L", "H"};
    private int speed;
    ...
    public void addCritter(Critter critter)
    {
        ...
        String critterDNA = toString(critter.getDNA());
        /*if (genePool.contains(critterDNA))
        {
            Color color = geneColors.get(genePool.indexOf(critterDNA));
            critter.setColor(color);
        }
        else
        {
            float r = (float)(.25 + Math.random() * .75);
            float g = (float)(.25 + Math.random() * .75);
            float b = (float)(.25 + Math.random() * .75);
            Color color =  new Color(r,g,b);
            genePool.add(critterDNA);
            geneColors.add(color);
            critter.setColor(color);
        }*/
    ...
    public void gameTimeStep()
    {
        ...
        while (i < critters.size())
        {
            ...
            if (iCritterEnergy <= 0 || iCritter.getAge() >= maxAge)
            {
                for (int j = 0; j < length; j++)
                {
                    barriers.remove(iBody[j]);
                    board.erase(new Point(iBody[j].x,iBody[j].y));
                }
                critters.remove(i);
            }
            else
